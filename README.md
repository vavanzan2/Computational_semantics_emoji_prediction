...
This repository contains the data processing pipeline developed for the course on Computational Semantics. 
The study investigates whether *conversational context* improves the ability of a pre-trained language model to predict how likely the emoji used (either artifically or spontaneously) is a given a text/turn.
...
The difference in predicted probabilities between these two conditions (Δ = p_context − p_no-context) serves as the operationalisation of *contextual effect* on emoji prediction.
...
The input file is a dataset that contains triad text-based conversations.  
